...
// サインアップボタンが押された場合
if (isset($_POST["newReserch"])) {
    // ユーザIDの入力チェック
    if (empty($_POST["researchName"])) {  // 値が空のとき
        $errorMessage = '調査名が未入力です。';
    } elseif (empty($_POST["targetName"])) {
        $errorMessage = '対象種名が未入力です。';
    } elseif (empty($_FILES['targetPicture'])) {
        $errorMessage = '対象種画像は必ず選択してください。';
    } else {
        ...
        try {
            ...
            // インサートする予定のResearchIDを取得
            $stmt = $pdo->prepare("SELECT * FROM research WHERE researchID = (SELECT MAX(researchID) FROM research)");
            $stmt->execute(array());
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            $researchID = $row['researchID']+1;

            $directory = 'researchID' . $researchID .'.jpg';
            try {
                if (!move_uploaded_file($_FILES["targetPicture"]["tmp_name"], 'tmp/research/' . $directory)) {
                    echo $_FILES["targetPicture"]["tmp_name"];
                    throw new Exception('画像ファイルアップロードエラー！');
                } else {
                    $_SESSION['targetPicture'] = $directory;
                }
            } catch (Exception $ex) {
                echo $ex->getMessage();
                // var_dump($_FILES["targetPicture"]["error"]);
                exit();
            }
        ...
        // 必須項目のチェック
        if (!empty($_POST["researchName"]) && !empty($_POST["targetName"])) {
            // 入力したユーザIDとパスワードを格納
            ...
            $_SESSION['researchOverview'] = htmlspecialchars($_POST['researchOverview'], ENT_QUOTES);
            $_SESSION['targetName'] = htmlspecialchars($_POST['targetName'], ENT_QUOTES);
            $_SESSION['targetOverview'] = htmlspecialchars($_POST['targetOverview'], ENT_QUOTES);
            $_SESSION['targetType'] = htmlspecialchars($_POST['targetType'], ENT_QUOTES);
            $_SESSION['researchType'] = htmlspecialchars($_POST['researchType'], ENT_QUOTES);

            if (isset($_POST['bingo'])) {
                $_SESSION['bingo'] = true;
            } else {
                $_SESSION['bingo'] = false;
            }

            // $errorMessage = $_SESSION['researchName'];
            header("Location: newresearch-detail.php");
            //var_dump($_FILES);
            exit;
        }
    }
...
if (isset($_POST['communityID'])) {
    $_SESSION['communityID'] = $_POST['communityID'];
} else {
    $_SESSION['message'] = "調査作成はコミュニティページから実施してください。";
...
if (isset($_SESSION["ID"])) {
    ...
    try {
        ...
        if (strcmp($row['userRole'], 'admin') != 0) {
            $_SESSION['message'] = '新規調査の作成はコミュニティ管理者のみ行うことが可能です。';
            header("Location: index.php");
            exit();
        }
    } catch (PDOException $e) {
    ...
    $_SESSION['message'] = '新規調査を実行するにはログインが必要です。';
...
EOM;
        ...
        <form action="" enctype="multipart/form-data" method="post">
            ...
            <div class="box-body">
              ...
              <div class="form-group has-warning">
                <div class="input-group date">
                  ...
                  <input type="text" name="researchName" class="form-control pull-right require">
                </div>
                <!-- /.input group -->
              </div>
              <!-- /.form group -->
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <textarea class="form-control" rows="3" placeholder="Enter ..." name="researchOverview"></textarea>
                </div>
                <!-- /.input group -->
              </div>
              <!-- /.form group -->
              ...
              <div class="form-group has-warning">
                <div class="input-group date">
                  ...
                  <input type="text" class="form-control pull-right require" name="targetName">
                </div>
                <!-- /.input group -->
              </div>
              <!-- /.form group -->
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <textarea class="form-control" rows="3" placeholder="Enter ..." name="targetOverview"></textarea>
                </div>
                <!-- /.input group -->
              </div>
              <!-- /.form group -->
                ...
                <div class="input-group">
                  <div class="row">
                    <div class="col-md-12">
                      <label class="">
                        <div class="iradio_minimal-blue" aria-checked="false" aria-disabled="false" style="position: relative;">
                          <input type="radio" name="targetType" checked class="minimal" style="position: absolute; opacity: 0;" value="multi">
                    ...
                    <div class="col-md-12">
                      <label class="">
                        <div class="iradio_minimal-blue" aria-checked="false" aria-disabled="false" style="position: relative;">
                          <input type="radio" name="targetType" class="minimal" style="position: absolute; opacity: 0;" value="mono">
                          <ins class="iCheck-helper" style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins></div>
                      </label>
                      単生
                    </div>
              ...
              <div class="form-group">
                <label>対象種画像</label>
                <small>　対象種の特徴がわかる画像を選択してください。200MB以下の画像のみアップロードできます。画像を選択しても調査項目設定へ進めない場合は、画像サイズを確認し再選択してください。</small>
                <div class="input-group">
                  <input type="hidden" name="MAX_FILE_SIZE" value="2097152">
                  <input type="file" id="media" accept="image/jpeg" name="targetPicture">
                </div>
                <!-- /.input group -->
              </div>
              <!-- /.form group -->
              ...
              <div class="form-group">
                <label>ビンゴ調査を行うか</label>
                <small>　※現在開発中の機能のため利用できません。</small>
                <div class="input-group">
                  <label>
                    <div class="icheckbox_minimal-blue" aria-checked="false" aria-disabled="false" style="position: relative;">
                      <input type="checkbox" class="minimal" style="position: absolute; opacity: 0;" name="bingo" value="1">
              ...
              <div class="form-group">
                <label>調査種類</label>
                <small>　継続調査を選択すると、調査は毎年自動的に更新され、新たに調査を作成せずに継続して調査を行うことができます。継続調査の停止はコミュニティページの調査一覧から可能です。単年度調査は、調査の自動更新をしない一度きりの調査を行う場合に選択してください。</small>
                <div class="input-group">
                  <div class="row">
                    <div class="col-md-12">
                      <label class="">
                        <div class="iradio_minimal-blue" aria-checked="false" aria-disabled="false" style="position: relative;">
                          <input type="radio" name="researchType" checked class="minimal" style="position: absolute; opacity: 0;" value="series">
                          <ins class="iCheck-helper" style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins></div>
                      </label>
                      継続調査
                    </div>
                    <div class="col-md-12">
                      <label class="">
                        <div class="iradio_minimal-blue" aria-checked="false" aria-disabled="false" style="position: relative;">
                          <input type="radio" name="researchType" class="minimal" style="position: absolute; opacity: 0;" value="single">
                          <ins class="iCheck-helper" style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></ins></div>
                      </label>
                      単年度調査
                    </div>
          ...
          <button type="submit" id="submit" name="newReserch" class="btn btn-block btn-success btn-lg" disabled>調査項目設定へ進む</button>
        </form>
      </section>
      <!-- /.content -->
    </div>
  ...
  <script>
    $(function() {
      ...
      $("#media").change(function() {
        //inputフィールドの文字数を取得
        fileCheck = $("#media").val().length;

        var image = $("#media").prop('files')[0];
        if (image.size >= 209715200) {
          fileCheck = 0;
          alert("200MB以下の画像のみアップロードできます");
        }
