        ...
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (strcmp($researchType, "single") == 0) {
                ...
                if ($private == 1) {
                    if ($doomsday < $now) {
                        // 調査情報アップデート
                        $stmt = $pdo->prepare('UPDATE research SET private = -1 WHERE researchID = ?');
                        $stmt->execute(array($researchID));
            ...
            // 全ての報告情報の取得（省略可能）
            $stmt = $pdo->prepare('SELECT * FROM report WHERE researchID = :ID && private = 1');
            $stmt->execute(array(':ID' => $researchID));
            ...
            $stmt = $pdo->prepare('SELECT *, report.reportID AS reportID FROM report LEFT JOIN user ON report.userID = user.userID LEFT JOIN report_media ON report.reportID = report_media.reportID WHERE (researchID = :ID) && (date LIKE :year) && (private = 1) ORDER BY date');
            $stmt->execute(array(':ID' => $researchID, ':year' => $researchYear . '%'));
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($agree > $disagree) {
                    ...
                    $comment .= '<a href="research-report-detail.php?id='. $researchID .'&report='. $row['reportID'] .'">詳細情報を確認</a>';
      ...
      <section class="content">
        ...
        <div class="box box-widget widget-user">
          ...
          <div class="widget-user-header bg-teal-active">
            <h3 class="widget-user-username"><?php echo $researchName; ?> -<?php echo $researchYear; ?></h3>
        ...
        <span class="custom-dropdown big">
          <select onChange="location.href=value;">
            ...
            <option selecte value="research-top.php?id=<?php echo $researchID; ?>">全期間</option>
            <?php
            // 重複したデータを除去して格納
            $year = array_unique($reportYear);
            foreach ($year as $key => $value) {
                echo '<option value="research-top.php?id='.$researchID.'&year='.$value.'">'.$value.'</option>';
            }
            ?>
          </select>
        </span>
        ...
        <div class="row">
          ...
          <div class="col-lg-3 col-xs-6">
            ...
            <div class="small-box bg-red">
              <div class="inner">
                <h3><?php echo $researchDate; ?><sup style="font-size: 20px"></sup></h3>
                <p>調査開始日</p>
              </div>
              <div class="icon">
        ...
        <div class="row">
          ...
          <div class="box">
            ...
            <div class="box-body">
              <div class="row">
                <div class="col-xs-6">
                  <button type="button" name="button" class="btn btn-primary btn-block btn-lg" onclick="location.href='search-top.php?id=<?php echo $researchID; ?>'">報告を行う</button>
                </div>
                <div class="col-xs-6">
                  <button type="button" name="button" class="btn btn-info btn-block btn-lg" onclick="location.href='research-report.php?id=<?php echo $researchID; ?>'">報告を見る・検証を行う</button>
                </div>
              </div>
            </div>
            <!-- /.box-body -->
          ...
          <section class="col-lg-4 connectedSortable">
            ...
            <div class="box">
              ...
              <!-- /.box-header -->
              <div class="box-body">
                <p>　下のQRコードを読み込むことでこのページを開くことが可能です。</p>
